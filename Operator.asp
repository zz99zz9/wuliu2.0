...
<div id="leftmenu">
<ul>
<li class="l1" onclick="javascript:location.href='Operator.asp'">操作员设置</li>
<li class="l" onclick="javascript:location.href='Operator_v.asp'">访问权限设置</li>
...
'news("Sup_OpeID")=Sup_OpeID '用户id
	 ...
	 news.close
	 set news=nothing
response.write "<script language='javascript'>alert('提交成功');location.href('operator.asp');</script>"
end if
'修改
if act="mod" then
'验证用户名是否存在
...
'news("Sup_OpeID")=Sup_OpeID '用户id
	 ...
	 news.close
	 set news=nothing
response.write "<script language='javascript'>alert('修改成功');location.href('operator.asp');</script>"
end if
...
sql="select * from operator where Ope_visitor=0 order by Ope_id"
